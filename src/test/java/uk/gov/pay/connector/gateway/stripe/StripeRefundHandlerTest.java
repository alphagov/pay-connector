...
public class StripeRefundHandlerTest {
    ...
    public void shouldRefundInFull() throws Exception {
        GatewayClient.Response refundResponse = mock(GatewayClient.Response.class);
        when(refundResponse.getStatus()).thenReturn(SC_OK);
        when(refundResponse.getEntity()).thenReturn(load(STRIPE_REFUND_FULL_CHARGE_RESPONSE));
        
        when(gatewayClient.postRequestFor(
                eq(refundsUri),
                eq(refundRequest.getGatewayAccount()),
                any(GatewayOrder.class),
                anyMap())).thenReturn(refundResponse);
    ...
    public void shouldNotRefund_whenAmountIsMoreThanChargeAmount() throws Exception {
        when(gatewayClient.postRequestFor(
                ...
                any(GatewayOrder.class),
                anyMap()))
                .thenThrow(new GatewayConnectionErrorException("Unexpected HTTP status code 402 from gateway", 402, load(STRIPE_REFUND_ERROR_GREATER_AMOUNT_RESPONSE)));
    ...
    public void shouldNotRefund_anAlreadyRefundedCharge() throws Exception {
        when(gatewayClient.postRequestFor(
                ...
                any(GatewayOrder.class),
                anyMap()))
                .thenThrow(new GatewayConnectionErrorException("Unexpected HTTP status code 402 from gateway", 402, load(STRIPE_REFUND_ERROR_ALREADY_REFUNDED_RESPONSE)));
    ...
    public void shouldNotRefund_whenStatusCode4xx() throws Exception {
        when(gatewayClient.postRequestFor(
                ...
                any(GatewayOrder.class),
                anyMap()))
                .thenThrow(new GatewayConnectionErrorException("Unexpected HTTP status code 402 from gateway", 402, load(STRIPE_ERROR_RESPONSE)));
