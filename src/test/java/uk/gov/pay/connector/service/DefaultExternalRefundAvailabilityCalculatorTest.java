...
public class DefaultExternalRefundAvailabilityCalculatorTest {
    ...
    public void historicCharge_shouldRecalculateAvailabilityIfCurrentAvailabilityIsMutable() {
        ...
        List<RefundEntity> partialRefunds = Arrays.asList(
                aValidRefundEntity().withStatus(RefundStatus.CREATED).withAmount(100L).build(),
                aValidRefundEntity().withStatus(RefundStatus.REFUND_SUBMITTED).withAmount(200L).build()
        );

        assertThat(defaultExternalRefundAvailabilityCalculator.calculate(getCharge(EXTERNAL_AVAILABLE), fullRefunds), is(EXTERNAL_FULL));
        assertThat(defaultExternalRefundAvailabilityCalculator.calculate(getCharge(EXTERNAL_FULL), partialRefunds), is(EXTERNAL_AVAILABLE));
    }
    ...
    public void historicCharge_shouldNotRecalculateAvailabilityIfCurrentAvailabilityIsTerminal() {
        ...
        assertThat(defaultExternalRefundAvailabilityCalculator.calculate(getCharge(EXTERNAL_UNAVAILABLE), refunds), is(EXTERNAL_UNAVAILABLE));
        assertThat(defaultExternalRefundAvailabilityCalculator.calculate(getCharge(EXTERNAL_PENDING), refunds), is(EXTERNAL_PENDING));
    }
    ...
    private Charge getCharge(ExternalChargeRefundAvailability availability) {
