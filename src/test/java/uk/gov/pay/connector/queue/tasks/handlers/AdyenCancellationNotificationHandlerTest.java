...
class AdyenCancellationNotificationHandlerTest {
    ...
    @ParameterizedTest
    @CsvSource({"false,USER CANCEL ERROR",
            "false,SYSTEM CANCEL ERROR",
            "true,USER CANCELLED",
            "false,USER CANCEL ERROR",
            "true,SYSTEM CANCELLED",
            "false,SYSTEM CANCELLED",
    })
    void shouldIgnoreCancelNotificationForConnectorCharge(Boolean success, String currentStatus) {
        when(mockAdyenNotificationService.deserialisePayloadToNotificationRequest(payload))
                .thenReturn(mockNotificationRequest);
        when(mockAdyenNotificationService.extractNotificationItems(mockNotificationRequest))
                .thenReturn(List.of(mockNotificationItem));
        ...
        assertThat(loggingEvents, everyItem(hasProperty("message", is("Ignored Adyen cancellation webhook"))));
    ...
    void shouldIgnoreCancelNotificationAndLogWarningWhenInUnexpectedState(Boolean success, String currentStatus) {
        ...
        assertThat(loggingEvents, everyItem(hasProperty("message", is("Unexpected state transition for Adyen cancellation webhook"))));
