...
import static uk.gov.pay.connector.paymentprocessor.service.CaptureProcessScheduler.CAPTURE_PROCESS_SCHEDULER_NAME;
import static uk.gov.pay.connector.paymentprocessor.service.CaptureProcessScheduler.INITIAL_DELAY_IN_SECONDS;
import static uk.gov.pay.connector.paymentprocessor.service.CaptureProcessScheduler.RANDOM_INTERVAL_MAXIMUM_IN_SECONDS;
import static uk.gov.pay.connector.paymentprocessor.service.CaptureProcessScheduler.RANDOM_INTERVAL_MINIMUM_IN_SECONDS;
import static uk.gov.pay.connector.paymentprocessor.service.CaptureProcessScheduler.SCHEDULER_THREADS;
...
public class CaptureProcessSchedulerTest {
    ...
    @Test
    public void shouldStartMultipleThreadsAsPerSchedulerThreadsConfiguration() {
        long initialDelayInSeconds = 0L;
        long randomIntervalMinimumInSeconds = 10L;
        long randomIntervalMaximumInSeconds = 10L;

        int schedulerThreads = 10;
        ...
        verify(scheduledExecutorService, times(1)).scheduleAtFixedRate(any(Runnable.class), eq(initialDelayInSeconds), delay.capture(), any());
    }
