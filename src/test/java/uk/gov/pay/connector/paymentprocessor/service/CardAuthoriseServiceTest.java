...
class CardAuthoriseServiceTest extends CardServiceTest {
    ...
    void doAuthoriseWeb_shouldRetainGeneratedTransactionId_WhenProviderAuthorisationFails() throws Exception {
        ...
        when(mockedPaymentProvider.generateTransactionId()).thenReturn(Optional.of(generatedTransactionId));
        mockExecutorServiceWillReturnCompletedResultWithSupplierReturnValue();
        when(mockedPaymentProvider.authorise(any(CardAuthorisationGatewayRequest.class), any(ChargeEntity.class)))
    ...
    void doAuthoriseWeb_shouldRespondAuthorisationCancelled_whenProviderAuthorisationIsCancelled() throws Exception {
        ...
        when(mockedChargeDao.findByExternalId(charge.getExternalId())).thenReturn(Optional.of(charge));
        double counterBefore = getMetricSample("gateway_operations_authorisation_result_total", new String[]{
                "sandbox", "test", "without-billing-address", "with non-corporate card", "not requested", "without email address", "without ip address", "authorisation error"
        });
    ...
    void doAuthoriseWeb_shouldReportAuthorisationTimeout_whenProviderTimeout() throws Exception {
        ...
        when(mockedChargeDao.findByExternalId(charge.getExternalId())).thenReturn(Optional.of(charge));
        double counterBefore = getMetricSample("gateway_operations_authorisation_result_total", new String[]{
                "sandbox", "test", "without-billing-address", "with non-corporate card", "not requested", "without email address", "without ip address", "authorisation timeout"
        });
    ...
    void doAuthoriseWeb_shouldReportUnexpectedError_whenProviderError() throws Exception {
        ...
        when(mockedChargeDao.findByExternalId(charge.getExternalId())).thenReturn(Optional.of(charge));
        double counterBefore = getMetricSample("gateway_operations_authorisation_result_total", new String[]{
                "sandbox", "test", "without-billing-address", "with non-corporate card", "not requested", "without email address", "without ip address", "authorisation unexpected error"
        });
    ...
    void doAuthoriseMotoApi_shouldRespondCaptureQueued_overridingGeneratedTransactionId() throws Exception {
        ...
        when(mockedChargeDao.findByExternalId(charge.getExternalId())).thenReturn(Optional.of(charge));
        double counterBefore = getMetricSample("gateway_operations_authorisation_result_total", new String[]{
                "sandbox", "test", "without-billing-address", "with non-corporate card", "not requested", "without email address", "without ip address", "authorisation success"
        });
    ...
    void doAuthoriseUserNotPresent_shouldRespondAuthorisationSuccess() throws Exception {
        ...
        double counterBefore = getMetricSample("gateway_operations_authorisation_result_total", new String[]{
                "worldpay", "test", "without-billing-address", "with non-corporate card", "not requested", "without email address", "without ip address", "authorisation success"
        });
    ...
    void doAuthoriseWorldpayUserNotPresent_shouldRespondAuthorisationFailed() throws Exception {
        ...
        double counterBefore = getMetricSample("gateway_operations_authorisation_result_total", new String[]{
                "worldpay", "test", "without-billing-address", "with non-corporate card", "not requested", "without email address", "without ip address", "authorisation rejected"
        });
    ...
    @Test
    void doAuthoriseWebWithWorldpayAuthoriseRequest_shouldRespondAuthorisationSuccess() throws Exception {
        when(mockMetricRegistry.counter(anyString())).thenReturn(mockCounter);

        ChargeEntity worldpayCharge = createNewChargeWith(1L, ENTERING_CARD_DETAILS, WORLDPAY);

        charge.setPaymentProvider(WORLDPAY.getName());
        ...
        when(mockedChargeDao.findByExternalId(worldpayCharge.getExternalId())).thenReturn(Optional.of(worldpayCharge));
        AuthCardDetails authCardDetails = AuthCardDetailsFixture.anAuthCardDetails()
                .withAddress(null)
                .build();

        AuthorisationRequestLog authorisationRequestLog = new AuthorisationRequestLog("Authorisation for…",
                List.of(kv(GATEWAY_REQUEST_RECORD, true)));
        when(mockWorldpayAuthoriseRequestLogGenerator.generate(authoriseRequest, authCardDetails)).thenReturn(authorisationRequestLog);

        AuthorisationResponse response = cardAuthorisationService.doAuthoriseWeb(worldpayCharge.getExternalId(), authCardDetails);

        assertTrue(response.getAuthoriseStatus().isPresent());
        assertThat(response.getAuthoriseStatus().get(), is(AuthoriseStatus.AUTHORISED));

        var loggingEvent = logs.assertContains("Authorisation for…");
        assertThat(loggingEvent.getArguments()
                        .stream()
                        .map(Object::toString)
                        .toList(),
                hasItem(GATEWAY_REQUEST_RECORD + "=true"));
    }
    ...
    private void providerWillRespondToAuthoriseWith(GatewayResponse value, PaymentGatewayName paymentGatewayName) throws Exception {
        when(mockedPaymentProvider.authorise(any(CardAuthorisationGatewayRequest.class), any(ChargeEntity.class))).thenReturn(value);
