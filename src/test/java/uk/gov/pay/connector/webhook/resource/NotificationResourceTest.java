...
class NotificationResourceTest {
    ...
    void shouldReturn200WhenAdyenNotificationSuccessfullyHandled() {
        ...
        when(adyenNotificationService.handleNotificationFor(anyString(), anyString(), eq(forwardedIpAddress))).thenReturn(true);

        try (Response response = notificationResource.authoriseAdyenPaymentsNotifications(rawNotification, forwardedIpAddress, hmacSignature)) {
    ...
    void shouldReturn403WhenAdyenNotificationValidationFails() {
        ...
        String hmacSignature = "sha256=test-signature";
        when(adyenNotificationService.handleNotificationFor(anyString(), anyString(), eq(forwardedIpAddress))).thenReturn(false);
