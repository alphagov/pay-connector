...
import uk.gov.pay.connector.resources.ChargeCaptureResource;
...
public class ConnectorApp extends Application<ConnectorConfiguration> {
    ...
    public void run(ConnectorConfiguration conf, Environment environment) throws Exception {
        ...
        environment.jersey().register(new CardDetailsResource(chargeDao, gatewayAccountDao));
        environment.jersey().register(new ChargeCaptureResource(chargeDao, gatewayAccountDao));
