...
public interface WorldpayOrderBuilder {
    ...
    static WorldpayOrderRequestBuilder buildAuthoriseOrder(CardAuthorisationGatewayRequest request,
        ...
        if (request.getGatewayAccount().isSendPayerIpAddressToGateway() && is3dsRequired && !request.getGatewayAccount().isAllowMoto()) {
            request.getAuthCardDetails().getIpAddress().ifPresent(builder::withPayerIpAddress);
        }

        if (request.getGatewayAccount().isSendPayerEmailToGateway() && is3dsRequired && !request.getGatewayAccount().isAllowMoto()) {
