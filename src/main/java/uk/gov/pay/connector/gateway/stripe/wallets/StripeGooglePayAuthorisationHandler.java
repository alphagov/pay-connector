...
public class StripeGooglePayAuthorisationHandler {
    private static final Logger logger = LoggerFactory.getLogger(StripeGooglePayAuthorisationHandler.class);
    private GatewayClient client;
    private StripeGatewayConfig stripeGatewayConfig;
    private String frontendUrl;
    private JsonObjectMapper objectMapper;

    public StripeGooglePayAuthorisationHandler(GatewayClient client,
    ...
    public GatewayResponse<BaseAuthoriseResponse> authorise(StripeGooglePayAuthorisationGatewayRequest request) throws GatewayException {
        GatewayResponse.GatewayResponseBuilder<BaseResponse> responseBuilder = GatewayResponse
                .GatewayResponseBuilder
                .responseBuilder();
        try {
            String paymentIntentJsonResponse = client.postRequestFor(StripePaymentIntentRequest.createPaymentIntentRequestWithToken(request, request.getTokenId(), stripeGatewayConfig, frontendUrl)).getEntity();
            ...
            logger.error("Uncaught exception thrown " + e.getMessage());
            return responseBuilder.withGatewayError(GatewayError.genericGatewayError(e.getMessage())).build();
        }
    }
