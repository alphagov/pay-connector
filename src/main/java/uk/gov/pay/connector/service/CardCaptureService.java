...
public class CardCaptureService extends CardService implements TransactionalGatewayOperation<BaseCaptureResponse> {
    ...
    @Inject
    public CardCaptureService(ChargeDao chargeDao, ChargeEventDao chargeEventDao, PaymentProviders providers, UserNotificationService userNotificationService, Environment environment, PaymentRequestDao paymentRequestDao) {
    ...
    public ChargeEntity markChargeAsCaptureApproved(String externalId) {
        return chargeDao.findByExternalId(externalId).map(charge -> {
            ...
            paymentRequestDao.updateChargeTransactionStatus(charge.getExternalId(), CAPTURE_APPROVED);
            return charge;
    ...
    public void markChargeAsCaptureError(String chargeId) {
        ...
        chargeDao.findByExternalId(chargeId).ifPresent(chargeEntity -> {
            ...
            paymentRequestDao.updateChargeTransactionStatus(chargeEntity.getExternalId(), CAPTURE_ERROR);
        });
    }
    ...
    public GatewayResponse<BaseCaptureResponse> postOperation(String chargeId, GatewayResponse<BaseCaptureResponse> operationResponse) {
        return chargeDao.findByExternalId(chargeId)
                .map(chargeEntity -> {
                    ...
                    chargeEventDao.persistChargeEventOf(chargeEntity, Optional.empty());
                    paymentRequestDao.updateChargeTransactionStatus(chargeEntity.getExternalId(), nextStatus);
                    ...
                    if (chargeEntity.getPaymentGatewayName() == PaymentGatewayName.SANDBOX) {
                        ...
                        paymentRequestDao.updateChargeTransactionStatus(chargeEntity.getExternalId(), CAPTURED);
                    }
