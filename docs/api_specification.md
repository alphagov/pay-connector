...
| `reference`              | X         | The reference issued by the government service for this payment                       |
| `description`            | X         | The payment description (shown to the user on the payment pages)                      |
| `created_date`           | X         | The date/time when payment was initiated by NHS BSA - ISO8601 including timezone      |
| `authorised_date`        | X         | The date/time when payment was authorised by Worldpay - ISO8601 including timezone    |
| `processor_id`           | X         | Unique ECKOH reference generated by ECKOH                                            |
| `provider_id`            | X         | The order code generated by Worldpay                 |
| `auth_code`              | X         | The authorisation ID generated by Worldpay                      |
| `payment_outcome.status` | X         | The payment status indicating successful capture. Either success or failed.                       |
| `payment_outcome.code`   | X         | The payment error code if capture is unsuccessful - required if payment_outcome.status is "failed"                       |
...
| `card_type`              |           | A valid card-type from master-card, visa, maestro, diners-club, american-express  |
| `name_on_card`           |           | The name on the card       |
| `email_address`          |           | The email address of the user       |
| `card_expiry`            |           | The card expiry date in MM/YY format       |
| `last_four_digits`       |           | Whether the payment requires an explicit request to capture — defaults to false       |
| `first_six_digits`       |           | Whether the payment requires an explicit request to capture — defaults to false       |
| `telephone_number`       |           | Whether the payment requires an explicit request to capture — defaults to false       |


### Response example when charge not in database
...
### Response example when charge in database
